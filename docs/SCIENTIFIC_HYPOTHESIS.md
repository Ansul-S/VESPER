...
| **Version** | 2.0 |
| **Revised** | 2026-06-15 (resolves gap-analysis F1, F2, F6; supersedes v1.0) |
| **Status** | Phase I — Scientific Validation (pre-registration, re-sealed) |
...
Here $R$ denotes completeness (recall) against injected transits in real TESS noise; "comb" is the combined fast-path + fallback system; "TLS" is full-grid TLS on every star. Both arms share identical preprocessing and an **identical TLS engine and detection threshold**, so that the only manipulated variable is whether the detector pre-screens and seeds the search (the fairness keystone; see [`TRINETRA_X_PHASE1_VALIDATION.md`](./TRINETRA_X_PHASE1_VALIDATION.md), §2).
...
- **A6 — Fair benchmark.** Using the same TLS engine and detection threshold in both arms compares the methods at **equal precision / false-alarm rate**, isolating recall and compute. *Check:* equalize thresholds to a common false-alarm rate on the calibration set ([`TRINETRA_X_PHASE1_VALIDATION.md`](./TRINETRA_X_PHASE1_VALIDATION.md), §2).
