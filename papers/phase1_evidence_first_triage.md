...
**DRAFT v0.2 — revised 2026-07-19.** Phase-I validation/methods paper. Grounded in the sealed M4 result (`research/m4_evaluation/M4_TEST_RESULT.md`) **as corrected by the audit erratum** (`research/m4_evaluation/M4_ERRATUM_2026-07-19.md`, authority DR-003): host-count correction (40, not 80), cluster-robust E1 inference, and the E2 re-measurement under the frozen timing rule. Citations key to `docs/references.bib` (verified against NASA ADS).
...
The occurrence-weighted recall difference is $\overline{\Delta R}=-0.48$ pp. Because all 15,000 injections share only 40 host noise realizations, we report the one-sided 95% lower bound under three constructions: the injection-level paired bootstrap ($-0.60$ pp), a **host-cluster bootstrap** that resamples the 40 hosts ($-0.82$ pp), and the pre-registered Wilson-style weighted combination ($-0.60$ pp). All clear the $-2$ pp margin decisively: **routing does not sacrifice recall.** On the occurrence-weighted scale (Table T2) recall is 0.218 (full) vs 0.213 (combined); on the unweighted injected grid it is 0.509 vs 0.488 (absolute figures optimistic per §2.3's residual-injection note). The weighted figures are low in absolute terms because the Kunimoto & Matthews (2020) weight concentrates on small ($R_p\le2$) planets, which are intrinsically hard; E1 concerns the *difference*, which is non-inferior.
...
The completeness maps (Figure F3) and Table T3 localize the structure. At $R_p=2\,R_\oplus$ — the weight-dominant class — the arms are at near-parity ($-0.37$ pp). The largest per-cell losses are at intermediate periods and larger radii ($R_p=4$: $-5.1$ pp; $R_p=8/12$: $-2.1/-2.7$ pp), which carry little occurrence weight. At $R_p=1$ both arms recover essentially nothing (noise-limited — too shallow to detect at any period in this baseline). Notably, at the shortest period ($P=0.5$ d) the combined arm **outperforms** full TLS by $+19$ pp (Table T3, by period; the blue column in F3). A dedicated control (erratum §6) shows the mechanism is *not* detection power and *not* a TLS grid-edge artifact (extending the grid below $P_{\min}$ changes nothing, per-row bit-identically): full TLS at $P=0.5$ d finds the right period at high SDE but fails the recovery predicate on **epoch** — its fitted $T_0$ falls outside $\pm0.5\,T_{14}$ in $\sim$95% of its failures there — while the combined arm's detector-seeded epoch survives the same predicate.
