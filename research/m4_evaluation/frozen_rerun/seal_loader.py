...
# Seal #2 digests. The TRINETRA-X -> VESPER rebrand (2026-06-30) rewrote the project
# name INSIDE the sealed manifest, changing its byte digest (DR-001 §5a). The post-
# rebrand file was verified byte-identical to the `phase1-prereg-v3` tag's version
# modulo pure branding strings (audit remediation 2026-07-19, DR-003), so BOTH digests
# are accepted: the original at the sealed tags, and the verified post-rebrand one.
# NOTE: from the rebrand until this fix, load_frozen() fail-closed on every call —
# the audit found the rebrand had silently broken the seal-verification chain.
SEAL2_SHA256S = {
    "6292c018c6923d512ac9c90dd55289cc010724d9facc27dc087f7e3f20832692",  # sealed (pre-rebrand)
    "5baf15df61fad9ddc236293baa7e8d446ec306d452e450ce895ec2ff9a453d38",  # post-rebrand, content-verified
}
...
def verify_seal() -> tuple[str, str]:
    ...
    if core_h not in SEAL2_SHA256S:
        raise SystemExit(f"[M4] FATAL: Seal #2 drift. got {core_h} expected one of {sorted(SEAL2_SHA256S)}")
